...
# Old version of Bottleneck block
...
# New version of Bottleneck block with IBN-a integration
class Bottleneck(nn.Module):
    expansion = 4
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None):
    ...
    def forward(self, x):
        ...
        if self.IN is not None:
            split = out.shape[1] // 2
            out_in = self.IN(out[:, :split, :, :])
            out_bn = out[:, split:, :, :]
            out = torch.cat((out_in, out_bn), 1)
...
class embed_net(nn.Module):
    ...
    def __init__(self, drop_last_stride,  decompose=False, modality_attention=0): 
        super(embed_net, self).__init__()
